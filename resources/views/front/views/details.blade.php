    ...
    <section class="max-w-[640px] w-full min-h-screen mx-auto flex flex-col bg-white overflow-x-hidden">
        ...
        <div class="flex flex-col z-30">
            ...
            <div id="content" class="w-full bg-white rounded-t-[40px] flex flex-col gap-5 p-[30px_24px_120px]">
                ...
                @forelse ($fundraising->phases as $phase)
                ...
                <div class="flex flex-col gap-[2px]">
                    ...
                    <p class="desc-less text-sm leading-[26px]">{{ substr($fundraising->about, 0, 100) }}
                        @if (strlen($fundraising->about) > 100)
                            <button class="text-[#FF7815] underline" onclick="toggleSeeMoreLess()">see more</button>
                        @endif
                    </p>
                    @if (strlen($fundraising->about) > 100)
                        <p class="desc-more text-sm leading-[26px] hidden">{{ $fundraising->about }}.
                            <button class="text-[#FF7815] underline" onclick="toggleSeeMoreLess()">see less</button>
                        </p>
                    @endif
                </div>
                        ...
                        @forelse ($fundraising->donaturs as $donatur)
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col gap-[2px] w-full">
                                    ...
                                    <p class="caption text-xs leading-[18px] text-[#66697A]">“{{ $donatur->notes }}”
                                    </p>
                                </div>
                            </div>
                        @empty
        ...
        @if (!$goalReached)
            <a href="{{ route('front.support', $fundraising) }}"
